...
namespace GTAEmblemMaker.Checks
{
    internal static class OfficialCatalogRoundChecks
    {
        internal static void Run()
        {
            ...
            CheckRound(OfficialCatalog.RoundBasis[0], "rounds/08", 211.872, 300.001, "7271704954196577C40DFD8546F28E1F68961D735C371E07F4FB239A26AE97FB", "M211.871,186.514C211.871,25.793,105.933,0.999,105.933,0.999S0,25.793,0,186.514L0.164,186.559C0.131,187.719,0,188.861,0,190.035C0,251.326,47.424,301,105.934,301C164.435,301,211.872,251.326,211.872,190.035C211.872,188.861,211.735,187.72,211.704,186.558L211.871,186.514Z");
            CheckRound(OfficialCatalog.RoundBasis[1], "rounds/18", 262.57, 300, "008AD95B4ED946D790F82B64ACADFE74C2CB7C8BC709B60CA8F36CD3B1062444", "M136.3,0H126.275H0C0,0,22.953,300,131.282,300C239.617,300,262.57,0,262.57,0H136.3Z");
        }

        private static void CheckRound(ShapeDefinition definition, string slug, double width, double height, string hash, string exportPath)
        {
            ...
            Check.Equal(width, exportDefinition.Width, slug + " Rockstar measured width");
            Check.Equal(height, exportDefinition.Height, slug + " Rockstar measured height");
            ...
            Check.Equal(Math.Round(width, 2, MidpointRounding.AwayFromZero), Convert.ToDouble(layer["width"]), slug + " Rockstar layer width");
            Check.Equal(Math.Round(height, 2, MidpointRounding.AwayFromZero), Convert.ToDouble(layer["height"]), slug + " Rockstar layer height");
